...
if (!supabaseUrl || !supabaseKey) {
  throw new Error('Folow Him is missing its Supabase connection settings. Please check Vercel environment variables.')
}
...
function bindAuth() {
  ...
  document.getElementById('authForm').onsubmit = async e => {
    ...
    try {
      ...
      if (result.error) {
        msg.textContent = formatAuthError(result.error)
        return
      }

      if (state.authMode === 'signup') {
        msg.textContent = result.data?.session
        ...
        msg.textContent = 'Sign-in completed, but no session was returned. Please try again.'
      }
    } catch (error) {
      msg.textContent = formatAuthError(error)
    }
  }
  ...
  document.getElementById('reset').onclick = async e => {
    ...
    if (!email) {
      msg.textContent = 'Enter your email address above, then click Forgot your password again.'
      input.focus()
      return
    }
    ...
    try {
      ...
      msg.textContent = error
        ? `Password reset could not be sent: ${formatAuthError(error)}`
        : 'Password reset instructions were sent. Check your email.'
    } catch (error) {
      msg.textContent = `Password reset could not be sent: ${formatAuthError(error)}`
    }
  }
}

function formatAuthError(error) {
  const raw = error?.message || String(error) || 'Authentication failed.'
  if (/invalid login credentials/i.test(raw)) return 'The email or password is incorrect. Try again or use Forgot your password.'
  if (/email not confirmed/i.test(raw)) return 'Please confirm your email address before signing in.'
  if (/api key|apikey|invalid.*key/i.test(raw)) return 'The deployed app is receiving an invalid Supabase client key. The database itself is reachable; check the Production Vercel key value.'
  if (/token.*not found|invalid.*token/i.test(raw)) return 'This confirmation or reset link has expired or was already used. Request a new email and use the newest link.'
  return raw
}
function bindApp(){
